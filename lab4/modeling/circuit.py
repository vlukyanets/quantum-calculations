...
from logic_value import LogicValue, logic_value_to_str
...
class Circuit:
    ...
    def replace_element(self, broken_element_name):
        broken_element_truthtable_name = self.elements[broken_element_name].truthtable.name
        if broken_element_truthtable_name is None:
            return None

        available_replaces = self.reserved_elements[broken_element_truthtable_name]
        if len(available_replaces) <= 0:
            return None

        replace_name = available_replaces[0]
        self.reserved_elements[broken_element_truthtable_name] = available_replaces[1:]

        element = self.elements[broken_element_name]
        del self.elements[broken_element_name]
        element.name = replace_name
        self.elements[replace_name] = element

        return replace_name

    def process_input_data(self, input_data):
        result_lines = []
        current_vector = [0] * len(self.input_lines)
        for input_data_item_tuple in input_data:
            input_data_type, input_data_item = input_data_item_tuple[0], input_data_item_tuple[1]
            if input_data_type == 0:  # Kill element
                new_element_name = self.replace_element(input_data_item)
                if new_element_name is not None:
                    result_lines.append("Replaced {0} with {1}".format(input_data_item, new_element_name))
                    self.prepare()
                    self.stimulate_by_vector(current_vector)
                    result_lines.append("Circuit state after replace:")
                    out = ''.join([logic_value_to_str(line.value) for line in self.lines])
                    result_lines.append(out)
                else:
                    result_lines.append("Cannot find replace for element {0}".format(input_data_item))
                    return result_lines
            else:
                current_vector = input_data_item
                self.stimulate_by_vector(current_vector)
                result_lines.append("Stimulate vector: " +
                                    ''.join([logic_value_to_str(line.value) for line in self.input_lines]))
                out = ''.join([logic_value_to_str(line.value) for line in self.lines])
                result_lines.append(out)
