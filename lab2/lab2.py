...
def read_input_data(filename):
    ...
    truthtable_header_re = re.compile(r'^Truthtable "(\S+)": (\d+) inputs, (\d+) outputs, (\d+) lines', re.IGNORECASE)
    truthtable_line_re = re.compile(r'^(\S+) (\S+)')
    element_re = re.compile(r'^Element "(\S+)": "(\S+)"; in (\S+); out (\S+)', re.IGNORECASE)
    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            if match and not inside_truthtable:
                vectors_str = match.group(0)
                global stimulating_vectors
...
def print_all_lines(circuit, f):
    out = [modeling.logic_value_to_str(line.value) for line in circuit.lines]
    print(out, file=f)
